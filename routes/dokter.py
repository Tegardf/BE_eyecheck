...
router = Blueprint("dokter",__name__)

@router.route("/", methods=["GET"])
@token_required(["dokter"])
def testrouter(test):
    return make_response(jsonify(message="test dokter route"),200)

@router.route("/new_rekaman", methods=["POST"])
@token_required(["dokter"])
def addRekaman(current_user):
    nama_pasien = request.json["nama_pasien"]
    tgl_lahir = request.json["tgl_lahir_pasien"]
    if (nama_pasien and tgl_lahir) == False:
    ...
    try:
        ...
        for data in data_pasiens:
            if (data.id == int(id_get)):
                rekaman.append(data)
        if len(rekaman)== 0:
            return make_response(jsonify(message = 'rekam medis tidak ada pada dokter'),400)
        rekaman = rekaman[0]
        all_data = {
            'id':rekaman.id, 
